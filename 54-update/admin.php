    ...
    <div class="modal m1">
        ...
        <div id="edit" class="mt-5">
            <form action="./api/edit.php" method="post" class="container" style="margin-top:100px;">
                ...
                <input type="text" name="first_name" class="form-group form-control " id="first_name" required
                    placeholder="First name（名字）">
                <label class="form-group col-3" for="last_name">Last name（姓氏）</label>
                <input type="text" name="last_name" class="form-group form-control " id="last_name" required
                    placeholder="Last name（姓氏）">
                <label class="form-group col-3" for="phone">Phone（手機號碼）</label>
                <input type="number" name="phone" class="form-group form-control " id="phone" required
                    placeholder="Phone（手機號碼）">
                <label class="form-group col-3" for="password">Password（會員密碼）</label>
                <input type="password" name="password" class="form-group form-control " id="password" required
                    placeholder="Password（會員密碼）">
                <input type="submit" style="width: 1110px;" class="btn btn-success" value="Edit(修改)"></input>
                <div class="btn btn-secondary mt-2" style="width: 1110px;" onclick="closemodal(1)">回上頁(Go back)</div>
            </form>
        </div>
    </div>
    <main style="width:80%;margin-left:auto;margin-right:auto;" id="look">
        <h1 class="text-center p-3" style="border:1px #888 solid;">購票資料 
            <a class="btn btn-success"href="./tickets.html">新增</a>
        </h1>
        <div style="overflow: scroll;height:600px;overflow-x:hidden;">
            <table class="table text-center table-striped">
                <tr style="background-color:#7090aa;color:#fff;height:50px;">
                ...
                <tr style="height:50px;" id="row_<?=$row['id']?>">
                    ...
                    <td style="height:50px;">
                        <button class="btn btn-secondary" id="edit<?=$row['id']?>"
                            onclick="edit('tickets',<?=$row['id']?>),openmodal(1);">修改</button>
                        <button class="btn btn-danger" id="btn<?=$row['id']?>"
                            onclick="$('#del<?=$row['id']?>').show();$('#btn<?=$row['id']?>').hide();$('#edit<?=$row['id']?>').hide();$('#by<?=$row['id']?>').show();$('#row_<?=$row['id']?>').css({'background':'#ff91917d','transform':'scale(1.01)','transition':'ease 0.5s'});">刪除</button>
                        <button style="display:none;" class="btn btn-outline-danger" id="del<?=$row['id']?>"
                            onclick="del('tickets',<?=$row['id']?>)">確認刪除</button>
                        <button style="display:none;" class="btn btn-outline-secondary" id="by<?=$row['id']?>"
                            onclick="$('#del<?=$row['id']?>').hide();$('#btn<?=$row['id']?>').show();$('#edit<?=$row['id']?>').show();$('#by<?=$row['id']?>').hide();$('#row_<?=$row['id']?>').css({'background':'#ffffffa3','transform':'scale(1)','transition':'ease 0.5s'});">取消</button>
                    </td>
                </tr>
                <?php
                }
